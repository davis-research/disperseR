...
for(i in 1:length(plotnames)){
  plotdimensions[i, "xrange"] <- getRange(expandedTrees[expandedTrees$plot==plotnames[i],"x"])
    plotdimensions[i, "yrange"] <- getRange(expandedTrees[expandedTrees$plot==plotnames[i],"y"])
}
