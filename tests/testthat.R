...
test_that("Results are expected", {
  expect_equal(findRotation(0,0,1,-1), 135)
  expect_equal(findRotation(0,2,0,0), 180)
...
test_that("Bad x/y values", {
  ...
  expect_warning(rotatePlot(data.frame(x=c(1:6), y=c(-4,0:-4), stringsAsFactors=FALSE)), "You have repeat")
